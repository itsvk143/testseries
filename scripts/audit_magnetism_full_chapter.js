...
async function runAudit() {
  ...
  console.log(`Connected to Atlas testseries database for audit of: "${CHAPTER}"\n`);

  const allDocs = await collection.find({ chapter: CHAPTER }).toArray();
  console.log(`=== 1. CHAPTER OVERVIEW ===`);
  console.log(`Total questions found: ${allDocs.length} (expected 584)`);
  ...
  const chapterDocIds = new Set(allDocs.map(d => String(d._id)));
  ...
  for (const paper of allPapers) {
    ...
    const matchedRefs = qIds.filter(id => chapterDocIds.has(id));

    if (matchedRefs.length > 0) {
      totalPapersReferencing++;
      totalReferencedQuestions += matchedRefs.length;

      // Verify each referenced doc actually exists
      for (const refId of matchedRefs) {
        if (!chapterDocIds.has(refId)) {
  ...
  console.log(`Total test papers referencing chapter: ${totalPapersReferencing}`);
  ...
  if (
    ...
    console.log(`\n>>> CHAPTER AUDIT PASSED 100%! ALL 584 QUESTIONS AND TEST PAPERS IN PERFECT STATE! <<<`);
  } else {
