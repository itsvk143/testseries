// scripts/audit_itf_mathematics.js
...
async function audit() {
  ...
  console.log('=====================================================');
  console.log('COMPREHENSIVE AUDIT: INVERSE TRIGONOMETRIC FUNCTIONS');
  console.log('=====================================================\n');
  ...
  if (allDocs.length !== 180) {
    console.error(`❌ Expected 180 documents, found ${allDocs.length}`);
    totalErrors++;
  } else {
    console.log(`✅ Correct total: exactly 180 documents.`);
  }
  ...
  if (typeCounts['MCQ'] !== 80 || typeCounts['ASSERTION_REASON'] !== 50 || typeCounts['NUMERICAL'] !== 50) {
    console.error(`❌ Type breakdown mismatch! Expected 80 MCQ, 50 AR, 50 NUM.`);
    totalErrors++;
  } else {
    console.log(`✅ Correct distribution: 80 MCQ, 50 AR, 50 NUM.`);
  }
  ...
  if (scoreErrors === 0) {
    console.log(`✅ All 180 questions have correct JEE Mains scoring (+4/-1 for MCQ/AR, +4/0 for NUM).`);
  } else {
  ...
  // 7. Duplicate Check across all 180 questions
  ...
  if (duplicateCount === 0) {
    console.log(`✅ All 180 questions are completely unique (0 duplicates).`);
  } else {
  ...
  const tpIds = [
    { id: new ObjectId('6a9e2845c527cd38431011bf'), name: 'Chapter Test (Inverse Trigonometric Functions)' },
    { id: new ObjectId('6a9e288dc527cd384310134a'), name: 'Subtopic Test (inverse-trigonometric-functions)' }
  ];
  ...
  for (const tpInfo of tpIds) {
    ...
    if (chapters.length !== 1 || !/inverse/i.test(chapters[0])) {
      console.error(`    ❌ Foreign chapter detected in test paper: ${JSON.stringify(chapters)}`);
      totalErrors++;
    } else {
      console.log(`    ✅ 100% pure Inverse Trigonometric Functions questions (no foreign chapters!).`);
    }

    // Check sections: 20 MCQs and 5 NUMs
    // To respect test order, map according to paper.questions order:
  ...
  if (repairCheckErrors === 0) {
    console.log(`✅ All 11 genuine repairs verified in MongoDB.`);
  } else {
