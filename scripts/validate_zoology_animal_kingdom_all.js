...
SUBTOPICS.forEach((subtopic, index) => {
  ...
  const filePath = path.join(__dirname, `data_zoology_animal_kingdom_part${partNum}.js`);
  ...
  data.forEach((q, qIdx) => {
    ...
    if (q.chapter !== 'Animal Kingdom') validationErrors.push(`${qLabel}: chapter mismatch "${q.chapter}"`);
...
if (validationErrors.length > 0) {
  ...
  console.log("SUCCESS: All 720 questions are 100% verified and ready for DB replacement!");
}
