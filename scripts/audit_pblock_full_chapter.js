...
async function auditChapter() {
  ...
  try {
    ...
    const allQs = await col.find({ chapter: "P-Block Elements" }).toArray();
    console.log(`Total questions in 'P-Block Elements': ${allQs.length} (Expected: 727)`);
    ...
    console.log(`  Genuine original Qs: ${genuine.length} (Expected: 142)`);
    console.log(`  Replaced authenticated Qs: ${replaced.length} (Expected: 585)`);
