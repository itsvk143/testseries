// scripts/audit_quadratic_mathematics.js
// Post-flight audit script for Quadratic Equations in MongoDB
...
async function main() {
  ...
  console.log('--- Auditing Quadratic Equations in MongoDB ---');
  const questions = await qb.find({ chapter: 'Quadratic Equations' }).toArray();
  console.log(`Total questions in questionBank for Quadratic Equations: ${questions.length}`);

  let errors = [];

  if (questions.length !== 301) {
    errors.push(`Expected 301 questions, found ${questions.length}`);
  }
  ...
  const bogusPatterns = [
    ...
    /\bbar\b/i,
    /conservation laws/i,
    /physical and quantitative relations/i
  ];
  ...
  // Audit test papers
  console.log('\n--- Auditing Quadratic Equations Test Papers ---');
  const testIds = [
    '6a9e2843c527cd38431011af',
    '6a9e2885c527cd3843101327',
    '6a9e2885c527cd3843101328'
  ];
  ...
  if (errors.length > 0) {
    ...
    console.log('\n🎉 ALL CHECKS PASSED 100%! QUADRATIC EQUATIONS IS FULLY AUDITED AND VERIFIED CLEAN!');
    process.exit(0);
  }
}
