// scripts/audit_straight_lines_mathematics.js
...
async function audit() {
  ...
  console.log("================ POST-FLIGHT AUDIT: STRAIGHT LINES ================");

  // 1. Total count
  const total = await questionBank.countDocuments({ chapter: "Straight Lines", subject: "Mathematics" });
  console.log(`Total questions in questionBank for Straight Lines: ${total}`);
  if (total !== 222) {
  ...
  const subtopicCounts = await questionBank
    .aggregate([
      { $match: { chapter: "Straight Lines", subject: "Mathematics" } },
  ...
  const bogusKeywords = [
    ...
    "Otto cycle", "refrigerator", "heat pump"
  ];
  let bogusCount = 0;
  ...
  const allDocs = await questionBank.find({ chapter: "Straight Lines", subject: "Mathematics" }).toArray();
  ...
  // 6. Test Paper audit
  console.log("\nAuditing Chapter Test Paper 'Straight Lines' (ID: 6a9e2844c527cd38431011b3)...");
  const testPaper = await testPapers.findOne({ _id: new ObjectId("6a9e2844c527cd38431011b3") });
  if (!testPaper) {
  ...
  let nonStraightLinesCount = 0;
  referencedDocs.forEach((d) => {
    if (d.chapter !== "Straight Lines" || d.subject !== "Mathematics") {
      console.error(`Foreign question in test paper: ID ${d._id}, chapter: ${d.chapter}, subject: ${d.subject}`);
      nonStraightLinesCount++;
    }
  });

  if (nonStraightLinesCount === 0) {
    console.log("Purity: 100% of test paper questions belong to Straight Lines (0 foreign questions)!");
  } else {
    process.exit(1);
  }
