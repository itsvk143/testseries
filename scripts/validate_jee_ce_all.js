...
for (let pIdx = 0; pIdx < partsConfig.length; pIdx++) {
  ...
  if (ar.length !== 26 || mcq.length !== 7 || num.length !== 20) {
    errors.push(`Part ${pIdx + 1} distribution mismatch: AR=${ar.length}, MCQ=${mcq.length}, NUM=${num.length}`);
  }
...
if (errors.length > 0) {
  ...
  console.log(`ALL 424 QUESTIONS VALIDATED SUCCESSFULLY! 0 ERRORS!`);
  console.log(`Schema, Marks (+4/-1), Types (208 AR, 56 MCQ, 160 NUM), and KaTeX fully verified.`);
}
