// scripts/psl/assemble_and_validate.js
const fs = require('fs');
const path = require('path');
const katex = require('katex');

const b1 = JSON.parse(fs.readFileSync(path.join(__dirname, 'psl_batch1.json'), 'utf8'));
const b2 = JSON.parse(fs.readFileSync(path.join(__dirname, 'psl_batch2.json'), 'utf8'));
const b3 = JSON.parse(fs.readFileSync(path.join(__dirname, 'psl_batch3.json'), 'utf8'));
...
allQuestions.forEach((q, i) => {
  q.questionId = `jee_mains_psl_${String(i + 1).padStart(3, '0')}`;
});

// Save assembled dataset
const outPath = path.join(__dirname, '../../src/data/properties_solids_liquids_275.json');
