// scripts/audit_diff_eq_mathematics.js
// Comprehensive post-flight audit script for Differential Equations in questionBank & testPapers.
...
async function audit() {
  ...
  console.log('\n--- 1. Auditing QuestionBank for Differential Equations ---');
  ...
  if (questions.length !== 220) {
    errors.push(`Expected exactly 220 questions, but found ${questions.length}`);
  }
  ...
  // 2. Audit Test Papers
  console.log('\n--- 2. Auditing Test Papers in testPapers ---');
  const testPaperIds = [
  ...
  for (const pId of testPaperIds) {
    const paper = await db.collection('testPapers').findOne({ _id: new ObjectId(pId) });
    if (!paper) {
      errors.push(`Test paper ${pId} not found!`);
      continue;
    }

    console.log(`\nAudit for Test Paper "${paper.title}" (${pId}):`);
  ...
  if (errors.length > 0) {
    ...
    console.log('\n>>> AUDIT PASSED WITH 0 ERRORS! Chapter Differential Equations is 100% verified and pristine. <<<');
    await client.close();
    process.exit(0);
  }
}
