...
// ==========================================
// SUBTOPIC 13: Electrochemical cells (25 AR, 8 MCQ, 271 NUM = 304)
// ==========================================
...
const electroChem = getElectrochemicalCellsQuestions();
console.log(`Electrochemical cells total questions: ${electroChem.length}`);

const mcqs = electroChem.filter(q => q.type === "MCQ");
const ars = electroChem.filter(q => q.type === "ASSERTION_REASON");
const nums = electroChem.filter(q => q.type === "NUMERICAL");
console.log(`  MCQs: ${mcqs.length}, ARs: ${ars.length}, Numericals: ${nums.length}`);

electroChem.forEach((q, idx) => {
  checkKatex(q.question, `Part4[${idx}].question`);
  q.options.forEach((o, oidx) => checkKatex(o, `Part4[${idx}].options[${oidx}]`));
  checkKatex(q.explanation, `Part4[${idx}].explanation`);
});
console.log("All Part 4 questions validated KaTeX successfully (0 errors)!");

// Write to scripts/data_redox_part4.js
const fileContent = `// Part 4: Authentic Questions for Redox Reactions and Electrochemistry
// Subtopic: Electrochemical cells (304 questions: 25 AR, 8 MCQ, 271 NUMERICAL)

${getElectrochemicalCellsQuestions.toString()}
${createMCQ.toString()}
${createAR.toString()}
...
fs.writeFileSync(__dirname + "/data_redox_part4.js", fileContent);
console.log("Written scripts/data_redox_part4.js successfully!");
