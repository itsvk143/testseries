// scripts/audit_matrices_mathematics.js
// Comprehensive audit of Matrices & Determinants questions and test papers in MongoDB
...
async function main() {
  ...
  const totalMat = await qb.countDocuments({ chapter: 'Matrices & Determinants' });
  console.log(`Total Matrices & Determinants questions: ${totalMat} (Expected: 251)`);
  if (totalMat !== 251) throw new Error(`Unexpected document count: ${totalMat}`);

  const bySubtopic = await qb.aggregate([
    { $match: { chapter: 'Matrices & Determinants' } },
  ...
  const allMatDocs = await qb.find({ chapter: 'Matrices & Determinants' }).toArray();
  let bogusMatches = 0;
  ...
  const testPaperIds = [
    '6a9e2844c527cd38431011b7',
    '6a9e2885c527cd3843101329',
    '6a9e2886c527cd384310132b',
    '6a9e96b8a28520078f5cc930'
  ];
  ...
  for (const tId of testPaperIds) {
    ...
    const nonMat = qDocs.filter(d => d.chapter !== 'Matrices & Determinants');
    if (nonMat.length > 0) {
      throw new Error(`Test paper ${tId} contains non-Matrices & Determinants questions!`);
    }
    console.log(`  ✅ All 25 questions belong to chapter "Matrices & Determinants".`);
  }

  // Also check Bayes' theorem cleanup
  const btPaper = await tp.findOne({ _id: new ObjectId('6a9e288ec527cd3843101350') });
  if (btPaper) {
    const btDocs = await qb.find({ _id: { $in: btPaper.questions } }).toArray();
    const nonProb = btDocs.filter(d => d.chapter !== 'Probability');
    console.log(`Checking Bayes' theorem test paper: ${btDocs.length} questions, Non-probability questions: ${nonProb.length}`);
    if (nonProb.length > 0) throw new Error("Bayes' theorem still has non-probability questions!");
    console.log(`  ✅ Bayes' theorem test paper is 100% clean.`);
  }
