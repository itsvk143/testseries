...
const bogusKeywords = [
  "adiabatic", "carnot", "rlc circuit", "magnetic flux", "photoelectric", "work function",
...
async function runAudit() {
  ...
  console.log("=== STEP 1: AUDITING ALL KINEMATICS QUESTIONS IN QUESTION BANK ===");
  const allQuestions = await qCol.find({ chapter: "Kinematics", subject: "Physics" }).toArray();
  console.log(`Total Kinematics questions fetched from DB: ${allQuestions.length}`);

  if (allQuestions.length !== 726) {
    throw new Error(`Expected 726 questions, got ${allQuestions.length}`);
  }
  ...
  const kinematicsIds = new Set(allQuestions.map(q => q._id.toString()));
  const allTests = await tCol.find({}).toArray();

  let referencingTests = 0;
  let totalKinematicsRefs = 0;
  let brokenRefs = 0;
  ...
  console.log(`Total test papers referencing Kinematics: ${referencingTests}`);
  console.log(`Total Kinematics question references in tests: ${totalKinematicsRefs}`);
  ...
  console.log(`All 726 questions verified, 0 KaTeX errors, 0 bogus questions.`);
  console.log(`All referencing test papers 100% operational.`);
  console.log(`========================================\n`);
