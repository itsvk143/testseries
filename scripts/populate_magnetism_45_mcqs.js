/**
 * scripts/populate_magnetism_45_mcqs.js
 * 
 * Synchronizes 45 authentic JEE Mains MCQs for each of the 7 Magnetic Effects of Current and Magnetism topics (315 total).
 * 
 * Updates:
 * 1. src/data/magnetism_315.json
...
const CHAPTER_NAME = "Magnetic Effects of Current and Magnetism";
...
async function main() {
  console.log('=== POPULATING MAGNETIC EFFECTS OF CURRENT & MAGNETISM 45 MCQS PER TOPIC ===\n');

  // 1. Load the 4 batches
  const b1Path = path.join(__dirname, 'magnetism', 'magnetism_batch1.json');
  const b2Path = path.join(__dirname, 'magnetism', 'magnetism_batch2.json');
  const b3Path = path.join(__dirname, 'magnetism', 'magnetism_batch3.json');
  const b4Path = path.join(__dirname, 'magnetism', 'magnetism_batch4.json');
  ...
  console.log(`Loaded ${allQuestions.length} total questions (B1: ${b1.length}, B2: ${b2.length}, B3: ${b3.length}, B4: ${b4.length})`);
  ...
  // 2a. src/data/magnetism_315.json
  const file315Path = path.join(process.cwd(), 'src', 'data', 'magnetism_315.json');
  fs.writeFileSync(file315Path, JSON.stringify({ Physics: { [CHAPTER_NAME]: magnetismMap } }, null, 2), 'utf8');
  ...
  currentJeem[CHAPTER_NAME] = magnetismMap;
  ...
  currentNeet[CHAPTER_NAME] = magnetismMap;
  ...
  const chapterQIds = [];
  // 7 subtopics: pick 4 from subtopics 0..3 (16) and 3 from subtopics 4..6 (9) => exactly 25 questions!
  ...
  const jeeChapterTestId = 'jee-mains-CHAPTER-Physics-Magnetic-Effects-of-Current-and-Magnetism-12';
  const neetChapterTestId = 'neet-CHAPTER-Physics-Magnetic-Effects-of-Current-and-Magnetism-12';
  ...
  await client.close();
  console.log('\n=== COMPLETED MAGNETISM POPULATION SUCCESSFULLY ===');
}
