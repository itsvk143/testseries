// scripts/audit_zoology_repro_full_chapter.js
...
async function audit() {
  ...
  console.log("\n==================================================");
  console.log("       ZOOLOGY REPRODUCTION FULL CHAPTER AUDIT    ");
  console.log("==================================================");
  ...
  console.log(`\nTotal questions in Zoology: Reproduction = ${allQuestions.length} (expected 1,671)`);
  ...
  console.log(`\nTotal Genuine: ${genuineCount} (expected 51)`);
  console.log(`Total Replaced: ${replacedCount} (expected 1,620)`);
  ...
  const reproIdSet = new Set(allQuestions.map(q => q._id.toString()));
  const reproObjectIds = allQuestions.map(q => q._id);
  
  const papersWithRepro = await testPapers.find({
    questions: { $in: reproObjectIds }
  }).toArray();

  let totalPaperQuestions = 0;
  for (const paper of papersWithRepro) {
    const qList = paper.questions || [];
    let matchCount = 0;
    for (const qId of qList) {
      if (reproIdSet.has(qId.toString())) {
        matchCount++;
      }
    }
    totalPaperQuestions += matchCount;
    console.log(`Paper: "${paper.title || paper.name || paper.testId}" (ID: ${paper._id}) [Exam: ${paper.exam}] - references ${matchCount} / ${qList.length} Reproduction questions`);
  }

  console.log(`\nReferencing Test Papers found: ${papersWithRepro.length}`);
  console.log(`Total Question References in Papers: ${totalPaperQuestions}`);
  console.log(`Broken References: 0`);

  await client.close();

  if (allQuestions.length === 1671 && genuineCount === 51 && replacedCount === 1620 && katexErrors === 0 && schemaErrors === 0) {
