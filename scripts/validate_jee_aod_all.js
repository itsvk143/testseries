// scripts/validate_jee_aod_all.js
const katex = require('katex');

const { repairedGenuineAOD } = require('./repaired_genuine_aod');
const sub1 = require('./data_jee_aod_subtopic1');
const sub2 = require('./data_jee_aod_subtopic2');
const sub3 = require('./data_jee_aod_subtopic3');
const sub4 = require('./data_jee_aod_subtopic4');
const sub5 = require('./data_jee_aod_subtopic5');
const sub6 = require('./data_jee_aod_subtopic6');

console.log('--- Loading All Modules for Application of Derivatives ---');
...
const genuineArray = Object.entries(repairedGenuineAOD).map(([id, q]) => ({
  ...normalizeItem(q),
  _id: id,
...
console.log(`Subtopic 1 (Increasing & Decreasing): ${sub1Norm.length} questions`);
console.log(`Subtopic 2 (Maxima & Minima): ${sub2Norm.length} questions`);
console.log(`Subtopic 3 (Monotonicity): ${sub3Norm.length} questions`);
console.log(`Subtopic 4 (Optimization): ${sub4Norm.length} questions`);
console.log(`Subtopic 5 (Inflection & Concavity): ${sub5Norm.length} questions`);
console.log(`Subtopic 6 (Rate of Change): ${sub6Norm.length} questions`);
...
if (errors > 0) {
  ...
  console.log('>>> SUCCESS: ALL 221 QUESTIONS PASSED KATEX, SCHEMA, AND ZERO-DUPLICATE CHECKS! <<<');
  process.exit(0);
}
