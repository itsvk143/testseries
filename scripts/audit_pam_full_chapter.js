...
async function audit() {
  ...
  const allQuestions = await qb.find({ chapter: "Physics and Measurement" }).toArray();
  console.log(`\n=== 1. QUESTION BANK AUDIT ===`);
  console.log(`Total questions in 'Physics and Measurement': ${allQuestions.length}`);
  ...
  for (const q of allQuestions) {
    ...
    if (q.type === 'NUMERICAL') {
      ...
      if (q.correctOptionIndex < 0 || q.correctOptionIndex > 3 || typeof q.correctOptionIndex !== 'number') {
  ...
  console.log(`\n=== 2. TEST PAPERS AUDIT ===`);
  const testsToVerify = [
    { testId: "jee-mains-CHAPTER-Physics-Physics-and-Measurement-11", expectedLen: 25 },
    { testId: "jee-mains-SUBTOPIC-Physics-Units-and-dimensions", expectedLen: 25 },
    { testId: "jee-mains-SUBTOPIC-Physics-error-analysis", expectedLen: 25 },
    { testId: "jee-mains-SUBTOPIC-Physics-significant-figures", expectedLen: 25 },
    { testId: "neet-CHAPTER-Physics-Physics-and-Measurement-11", expectedLen: 45 },
    { testId: "neet-SUBTOPIC-Physics-Units-and-dimensions", expectedLen: 45 },
    { testId: "neet-SUBTOPIC-Physics-error-analysis", expectedLen: 43 },
    { testId: "neet-SUBTOPIC-Physics-significant-figures", expectedLen: 43 }
  ];
  ...
  for (const t of testsToVerify) {
    ...
    // Verify all question IDs exist in DB and check types
    let mcqCount = 0;
    let arCount = 0;
    let numCount = 0;
    let missingCount = 0;
