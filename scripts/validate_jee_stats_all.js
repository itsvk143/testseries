// scripts/validate_jee_stats_all.js
const katex = require('katex');

const { repairedGenuineStatistics } = require('./repaired_genuine_statistics');
const { subtopic1Questions } = require('./data_jee_stats_subtopic1');
const { subtopic2Questions } = require('./data_jee_stats_subtopic2');
const { subtopic3Questions } = require('./data_jee_stats_subtopic3');
const { subtopic4Questions } = require('./data_jee_stats_subtopic4');
const { subtopic5Questions } = require('./data_jee_stats_subtopic5');
...
const sets = [
  { name: 'Repaired Genuine (49)', questions: Object.values(repairedGenuineStatistics).map(q => ({ type: 'MCQ', marks: 4, negativeMarks: 1, ...q })), expected: 49 },
...
console.log('\n========================================');
console.log(`Total questions checked: ${totalQuestions} (Target: 199)`);
console.log(`Unique questions: ${seenQuestions.size}`);
console.log(`Total errors: ${errorCount}`);
if (errorCount === 0 && totalQuestions === 199) {
  console.log('✅ ALL 199 QUESTIONS PASSED 100% VALIDATION WITH ZERO ERRORS!');
  process.exit(0);
} else {
