// scripts/audit_reproduction_full_chapter.js
...
const BOGUS_PATTERNS = [
  ...
  /capacitance/i,
  /resistor/i,
  /governing variables/i,
  /restoring force/i,
  /characteristic interval/i
];
...
async function runAudit() {
  ...
  try {
    ...
    console.log('--- AUDITING REPRODUCTION IN PLANTS (BOTANY) ---');
    ...
    if (questions.length !== 900) {
      console.warn(`WARNING: Expected 900 questions, got ${questions.length}`);
    }
          ...
          if (sec.questions && Array.isArray(sec.questions)) {
            for (const qRef of sec.questions) {
              ...
              if (!questionIdSet.has(qIdStr)) {
                // If the question is outside this chapter, that's normal for full mock tests
              }
            }
          }
        }
    ...
    console.log('--- AUDIT COMPLETE: ALL 900 QUESTIONS 100% CLEAN ---');
