// scripts/audit_genetics_full_chapter.js
// Comprehensive audit of all questions in Botany -> Genetics and Evolution
...
async function audit() {
  ...
  console.log("\n--- Step 1: Auditing Entire Question Bank for Genetics and Evolution ---");
  const allQuestions = await qb.find({ subject: "Botany", chapter: "Genetics and Evolution" }).toArray();
  console.log(`Total questions retrieved: ${allQuestions.length} (Expected: 1492)`);
  ...
  for (const paper of testPapers) {
    ...
    for (const qid of qIds) {
      const exists = await qb.findOne({ _id: new (require('mongodb').ObjectId)(qid) });
      if (!exists) {
    ...
    console.log(`Paper: "${paper.title}" -> ${qCount} valid question references verified.`);
  }

  console.log(`\nBroken test paper references: ${brokenRefs}`);

  console.log("\n====================================");
  if (totalErrors === 0 && brokenRefs === 0 && allQuestions.length === 1492) {
    console.log("FINAL RESULT: ALL 1,492 QUESTIONS AND ALL TEST PAPERS ARE 100% HEALTHY AND VERIFIED!");
  } else {
