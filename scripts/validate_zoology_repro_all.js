// scripts/validate_zoology_repro_all.js
const fs = require('fs');
...
const SUBTOPICS = [
  ...
  "Parturition, lactation, and embryonic development",
  "Reproductive health",
  "Spermatogenesis, oogenesis, and hormonal regulation"
];
...
SUBTOPICS.forEach((st, idx) => {
  const partNum = idx + 1;
  const filePath = path.join(__dirname, `data_zoology_repro_part${partNum}.js`);
  ...
  data.forEach((q, qIdx) => {
    ...
    if (q.chapter !== "Reproduction" || q.subject !== "Zoology") {
...
console.log(`Total Assertion-Reason: ${totalAR} (expected: ${26 * 9} = 234)`);
console.log(`Total MCQ: ${totalMCQ} (expected: ${154 * 9} = 1386)`);
console.log(`Schema errors: ${schemaErrors}`);
console.log(`KaTeX errors: ${totalKatexErrors}`);

if (schemaErrors === 0 && totalKatexErrors === 0 && totalQuestions === 1620 && totalAR === 234 && totalMCQ === 1386) {
  console.log("\n>>> ALL 1,620 QUESTIONS VALIDATED PERFECTLY WITH ZERO ERRORS! <<<");
} else {
